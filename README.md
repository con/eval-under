...
| Backend | git-annex test | git testsuite | stress-ng | mtime stability | git-annex linkAnnex loop | pjdfstest |
| --- | --- | --- | --- | --- | --- | --- |
...
[`gh-pages`](../../tree/gh-pages) branch, and **each badge links to that
cell's own job log** via the [status page](https://con.github.io/eval-under/) --
...
# Same, with client-side attribute caching off -- the knob that decides
# whether git-annex's "failed to link to annex" / "unlock failed" on NFS
# is the filesystem or the code (con/git-annex#293)

...
Generated from [`evals/known-issues.yaml`](evals/known-issues.yaml) by `bin/ci/known_issues.py gotchas`.
...
Working hypothesis: our git-annex build lacks OsPath support, which
upstream's builds have.
...
Backends listed explicitly so a new loop-* row does not inherit a
whole-cell issue.

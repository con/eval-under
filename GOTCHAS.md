...
entry in [`evals/known-issues.yaml`](evals/known-issues.yaml), which
...
**Expect:** fail \
**Tags:** `build-config`, `fixed-upstream`, `needs-triage`
...
cells log well over a hundred "not enough free space" messages.
Hypothesis, not yet confirmed: that is also why the ext4 control row
fails -- only ~61 of its ~466 failure messages mention free space
directly; the rest look like knock-on failures, but no run has shown
it. Whole-cell (so this row cannot catch regressions!) until the
image is enlarged or diskreserve lowered for the test run, which
will settle it and reveal what is genuinely vfat-specific. Backends
are listed explicitly so a new loop-* row does not inherit this.
...
it the prime suspect for the "pre-existing" `Loop ext4 / git-annex test`
failure -- a property of our harness, not of ext4 or vfat -- but it is
not proven: only ~61 of the ~466 ext4 failure messages mention free
space directly. `loop-annex-diskreserve` (tags `harness`,
`needs-triage`) covers both loop rows whole-cell until the image is
enlarged or the reserve lowered for the test run. Until then the ext4
